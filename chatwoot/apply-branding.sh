...
echo ""
echo "--- favicon ---"
curl -sI "https://letsm.io/favicon.ico" 2>&1 | grep -iE "HTTP|content-type|content-length" | head -3
echo ""
echo "--- custom-branding.css ---"
curl -sI "https://letsm.io/custom-branding.css" 2>&1 | grep -iE "HTTP|content-type" | head -2
echo ""
echo "--- HTML <title> ---"
curl -s "https://letsm.io/" 2>&1 | grep -oE "<title>[^<]*</title>" | head -1
